...
2026-09-25: 実機目視 1 巡目（#82 の 10 項目）を行った。main は **`0463cbb`**（#121 = PR #124・#122 = PR #125 を統合済み。この記録は #123 / PR #126）。
統合チェックリストは 96 項目、結果は 82 番台 9 行（82-9 は空欄）と 75-21 / 76-15 の合わせて 13 行。
コンボの枠が OS の色のまま（#118）・失敗の箱に音と印が欲しい（#119）・保存できないノートから抜ける道が無い（#120）・
設定でライトにしても絞り込み欄がダークのまま（#121）・English で「操作 ▾」が省略される（#122）を新しい Issue にした。
**開いている Issue は #70 / #42 / #112 と #118〜#123。着手の順（hide）は #121 #122 → #120 → #119 → #118 → #70 → #42。**
最新は[日報](docs/reports/2026-09-25.md) / [引き継ぎ](docs/handoffs/2026-09-25.md)。
