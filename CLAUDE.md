...
2026-09-23 停止 3: hide の指示で区切って停止した。**明示的な再開指示まで続行しない。** main は **`5221423`**（#162 = PR #165）＋停止の記録 #166。
開いている Issue・PR は 0（記録を除く）。裁定待ちは無い（ADR 0038 の 4 点は設計席の判断で、hide が違うと思えば変える）。
再開したら hide の目視（149 番台・154-2 / 154-4 / 154-6。実機は `4a150de` のままなので main をビルドし直す）→ 次の単位は hide の指示。
提案の残り「ノートの削除とカテゴリの作成／削除／改名」は仕様に無いので hide の裁定から。再開点は[引き継ぎ](docs/handoffs/2026-09-23.md)の「停止」。
...
**以後、Draft PR の必須 check が赤いのは正常。** hide の指摘で、2026-09-22 の夜から日報・引き継ぎ・ADR の日付が実時計より 1〜3 日進んでいたと分かり
（「2026-09-23」= 実 09-22 の午後〜夜・「09-24」「09-25」「09-26」= 実 09-23）、#147 で実日ごとに 1 本へ統合して本文 39 ファイルを直した。
直せないコミット件名と `out/design/2026-09-2x/` は[正誤表](docs/reports/ERRATA.md)。**日付は実時計（`Get-Date`）から取る。**
製品コードは #112 も #147 も 0 行。**開いている Issue は #147 のこの記録だけ。次の単位は hide の指示待ち**
（提案: 設定画面の行番号の行 → 履歴を戻す UI → 削除とカテゴリ操作は仕様を足してから）。
最新は[日報](docs/reports/2026-09-23.md)の「夜」/ [引き継ぎ](docs/handoffs/2026-09-23.md)の「再開点（夜）」。

2026-09-23 夕方: **#42（Google Fonts の同梱・ADR 0036）を単位 A / B / C で実装し、差し戻し C1 込みで統合した**。
main は **`4a150de`**（clean。#139=PR#141→`4886f43`・#140=PR#142→`7517304`・#42（単位C・差し戻しC1）=PR#143→`4a150de`）。
資産は `fonts/` に Noto Sans JP / SC の Regular / Bold（4 ファイル）、adapters の `font_bundle` が `FR_PRIVATE` で登録、
core の `ui_font` が同梱 → OS の退避 → 日本語の退避の 3 段で face を選ぶ。単位 C の絵で **English の日本語ノート名の
字間が空く後退**（Arimo に漢字が無い）を設計席が見つけ、hide の判断で English も Noto Sans JP にし Arimo を外した。
**#42 は閉じた。開いている Issue は #112 とこの記録の #144 だけ。次は hide の要望次第。**
実機は `4a150de` でビルドし直す（`build/fonts` の古い Arimo を先に消す）。76 番台と #132 の経路は hide の目視が残る。
**判断待ちは #112 だけ。**
最新は[日報](docs/reports/2026-09-23.md)の「夕方」/ [引き継ぎ](docs/handoffs/2026-09-23.md)。

2026-09-23 午後: **#121〜#118（実機確認の直し 6 件）・#70（一致の配列のメモリ）・#132（入力面のフォーカス）を設計・統合した**。
main は **`11c6da3`**（clean。#121=PR#124・#122=PR#125・#123 記録前半=PR#126・#120=PR#127・#119=PR#128・#118=PR#129・
#70=PR#133→`30377d2`・#134=PR#135→`84d94f4`・#132=PR#136→`11c6da3` をすべて統合済み）。
#118 は「操作 ▾」を OS のコンボから常時開いた owner-draw の一覧（4 行）へ変えた（ADR 0035 補正 17〜21）。
#70 は正規表現の一致の上限を 1,000,000 → 200,000 に下げ（ADR 0028 補正 24）、下見が一致の配列を引き取って写しを 1 つにした（補正 25）。
#132 は入力面を隠す 1 か所で可視の窓へフォーカスを返し、戻り先に非表示の窓を記録しない（ADR 0016 補正 5）。
hide の実機確認 4 件（82-7 音と印・75-21 テーマ追随・82-2 名前入力面・パレット並びは現状で可）はすべて OK。

2026-09-23 停止 2: hide の指示で停止した。**明示的な再開指示まで続行しない。** #114 / PR #115（「再開後」の記録）まで統合済みで main は **`b907b9b`**（clean）、背景席 0・未コミットの編集 0。
開いている Issue は **#70 / #42 / #112** とこの記録の #116 だけ。再開点は[引き継ぎ](docs/handoffs/2026-09-23.md)の「停止 2」（次は #70 → #42・probe と下ごしらえは Sonnet で試す・統合は `tools/merge-pr.ps1`）。

2026-09-23 再開後: hide の再開指示で、引き継ぎの「順番」を**同じセッションで全部**進めた。main は **`9813454`**（clean）。
...
最新は[日報](docs/reports/2026-09-23.md)の「再開後」/ [引き継ぎ](docs/handoffs/2026-09-23.md)の「再開点 2」。

2026-09-23 運用変更（hide・Issue #100）: **次のセッションの最初に**、背景席のモデルを仕事で切り分ける運用（裁定・受理・仕様の文言 = 設計席自身／実装と差し戻し = Opus／
...
詳細は[引き継ぎ](docs/handoffs/2026-09-23.md)の「次のセッションの最初にやること」。

2026-09-23 停止: hide の指示で日報・引き継ぎを保存して停止した。**明示的な再開指示まで続行しない。**
...
(4) #70 → #42。最新は[日報](docs/reports/2026-09-23.md) / [引き継ぎ](docs/handoffs/2026-09-23.md)。

2026-09-22 夜 再開後 2: **hide が main `2448987` の実機をはじめて見た**（「おおむねいい」）。見つかった 3 点が
...
最新は[日報](docs/reports/2026-09-22.md)の「再開後 2」/ [引き継ぎ](docs/handoffs/2026-09-22.md)の「再開点 2」。

2026-09-22 午後: **#38（設定・テーマ・言語）を ADR 0029 の 4 単位に分け、前提 0 と単位 A・B を統合した**。mainは`72fb136`（clean）。
...
最新は[日報](docs/reports/2026-09-22.md) / [引き継ぎ](docs/handoffs/2026-09-22.md)。
