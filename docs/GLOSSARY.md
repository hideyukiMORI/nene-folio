...
| 失敗の箱（failure box） | 「知らせて戻る」だけのモーダル。application の `folio_state_failure_line` の 1 行を、名前入力面と同じ `dialog_theme` の塗りで見せる。答えで分岐しない。作れないときだけ `MessageBoxW` へ退避 | `failure_box_show`（ui/win32）・ADR 0035 |
...
| `ui_font` | 言語ごとの書体の face 名を持つ core の表 1 か所。`ui_font_face(language)` が引く |
