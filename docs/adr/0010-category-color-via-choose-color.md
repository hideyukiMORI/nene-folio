...
- 実機は DPI 120 の合成入力でだけ見る。`ChooseColorW` の中は OS の部品なので検証の対象にせず、初期値とカスタム色の写真を 1 枚だけ残す（`color-dialog.png`）
- 実装で足した判断（2026-09-10・PR の前に記録）: カスタム色 16 個は白（`RGB(255, 255, 255)`）で始める（黒は「未設定」に見えない）。
  `category_ledger_toggled` と `recolored` の「全項目を複製する」部分は `category_ledger.c` の 1 つの静的関数に括り出した（ARC-001）
