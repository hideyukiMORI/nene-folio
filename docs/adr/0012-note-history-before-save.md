...
- ローテーションは原子的でない。2 手目以降で落ちれば番号が欠けたり `1.md.tmp` が残ったりするが、md は無傷。`.tmp` の掃除は
  持たない（要るなら「起動時に `.history` の `*.tmp` を消す」を別 Issue）。「削除は通るが改名は拒む」状態は NTFS の ACL で作れず未実測
- 控えの書き込みに `file_bytes_store` をそのまま使うので、一瞬 `1.md.tmp.tmp` が現れる（書き込みの正典経路を 2 本にしない選択）
