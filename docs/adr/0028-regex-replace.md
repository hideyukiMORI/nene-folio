...
## 2026-09-25 の補正（#70・決定本文は書き換えない）

記録 23 の懸念を実測した（`out/design/2026-09-25/replace-memory-probe/`・`GetProcessMemoryInfo` の PeakWorkingSet・実アダプタ・DPI や UI は無関係）。
1MB の本文で `\w+`（169,120 件）の下見は **+77.5MB**、900KB の本文で `.`（907 千件・上限直下）は **+403.6MB**（机上の 232 バイト × 2 写し = 401.2MB と一致）、
1MB の `.`（上限超）は走査が打ち切られるので +0.65MB。危ないのは**上限直下で成功する走査**である。`found` は伸びたら縮まず、下見の写しは打鍵ごとに作り直される（Private Usage は下見の分だけ減る）。
現物調査は `out/agents/70-probe/memo.md`。

24. **`regex_match_limit` を 1,000,000 から 200,000 に下げる。** 1 写しが 200,000 × 232 = 46.4MB で、写しが 2 つでも 93MB に収まる。
    1MB × `\w+` の 10 万件台（実測 108,773 / 169,120 件）は通る。上限の直下で成功する走査が 400MB を要する現状を、値 1 つで止める。
    変わる振る舞い: `.` のような 1 文字ごとの一致は約 200KB を超える本文で `TOO_MANY` になる（これまでは約 1MB）。文言（`REPLACE_TOO_MANY`）は件数を言っていないので変えない。
    `tests/unit/replace_tests.c` の固定値の検査は新しい値に直す。出力の上限 `note_replace_limit`（4,000,000 単位）は動かさない。
25. **写しを 1 つにするのは次の単位**（同じ #70 の 2 本目の PR）。下見（`replace_preview_create`）が `found` の配列を `memcpy` で写す代わりに**配列ごと引き取り**、
    application は `found` を空に戻して次の走査で数えてから確保し直す（`scan_matches` は 2 周で件数どおりに確保するので倍々の無駄は無い）。
    定常は「新しい `found` ＋ 直前の下見」の 2 つで、打鍵ごとの 46MB の `memcpy` が消える。`note_replace_build` は参照で読むので触らない。
    下見と `found` の同一性の照合（`replace_preview_holds`）は本文と宛先で行っており、配列の所有者が変わっても影響しない。24 を統合して実測してから着手する。
26. 群を present なものだけ持つ疎な形（Issue の候補 (c)）は**採らない**。`struct regex_match` を完全型で公開する根拠（決定 2・C-003 の例外「全メンバーが独立に妥当」）と衝突し、
    偽の adapter のテストの初期化子を全部書き換えることになる。
