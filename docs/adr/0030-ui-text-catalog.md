...
## 2026-09-22 の補正 2（#88 / ADR 0033・決定本文は書き換えない）

10. **決定 1 の群から `GLYPH` が消える。** `UI_TEXT_GLYPH_MINUS` / `UI_TEXT_GLYPH_PLUS` は
    ドロワーの折畳印にしか使われていなかった。ADR 0033 でその印が 24 の viewBox の面のパス
    （`ICON_PAINT_FOLD_COLLAPSE` / `ICON_PAINT_FOLD_EXPAND`）になったので、**単独で描く字形は 1 つも残らない**。
    列挙・表・単体の期待表の 3 か所から 2 値を消し、`ui_text.h` の冒頭の群の一覧からも `GLYPH` を外した。
    `UI_TEXT_APP_NO_WINDOW` が列挙の最後であることは変わらないので `expected_count` の不変は保たれ、
    CNF-009 の `lineTables`（`ui_text.h` ↔ `ui_text.c`）は設定を変えずに整合する。
11. **補正 4（`UI_TEXT_GLYPH_MINUS` を UTF-8 の実体 U+2212 で持つ）は、値ごと無くなったので効力を失う。**
    記録としては残すが、`−` の符号位置を気にする場所はもう無い。折畳の印は文字ではなく面である。
