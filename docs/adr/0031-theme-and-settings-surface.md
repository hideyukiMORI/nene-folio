...
- 状態: 受理（設計リナ 2026-09-22。ADR 0029 の単位 B。実測 `out/design/2026-09-22/theme-probe/`（再着色 7 案・RTF の流し直し・`WM_SETTINGCHANGE`・DWM・レイヤーのフォーカスと IME・配色のコントラスト）に基づき、現行コード（main `f881f16`・#74 統合後）に照らした読み取り専用の批評（止める所見 7・直したい所見 10）を経て直した）
- 日付: 2026-09-22
...
結果のまとめは[確認記録](../quality/2026-09-22-theme-checks.md)。
...
## 2026-09-22 の補正 2（ADR 0032・決定本文は書き換えない）
...
## 2026-09-22 の補正 3（#88 / ADR 0033・決定本文は書き換えない）
...
## 2026-09-23 の補正 3（ADR 0035・決定本文は書き換えない）

決定 10 で「別 Issue」にした **`name_prompt` と失敗の箱のテーマ追随は #82 / [ADR 0035](0035-dialog-theme.md) で実装した**
（comctl32 v5 のまま `dialog_theme` の 1 本で塗り、失敗の箱は `MessageBoxW` をやめて同じ塗りの自前のモーダル。面のテーマは開く瞬間に固定）。
決定 10 の「OS 既定色のまま」はこれで解ける。`ChooseColorW` の窓の色は据え置き。

## 2026-09-23 の補正 4（#149・行番号の段・決定本文は書き換えない）

FR-021 の「設定画面からの変更は #38 が同じ意図へ結ぶ」が未達だった（設定画面は テーマ・言語 の 2 段のまま）。
#149 で **3 段目に「行番号」の段**（見出し 1 行 ＋「表示する」「表示しない」の 2 行）を足す。

1. 行の種別に `SETTINGS_ROW_NUMBER` を足し、`value` は `bool` の 1 / 0。段の順は **テーマ → 言語 → 行番号**（既存の行の位置は動かさない）。
2. 採用は既存の `apply_number`（`folio_state_set_number` → 先に書いてから採用・ADR 0025）1 本で、パレット・Ex・メニューと**同じ意図**を通す（ARC-001）。
   `apply_theme` と同じく、**設定画面から採用したときは面を閉じず**印だけ移し（決定 7 の「欄は開いたまま」）、他の入口からは従来どおり入力面を閉じる。
   採用のあとは `rearrange_keeping_line` で番号の帯を出し入れする（読んでいる論理行を保つ・ADR 0026）。
3. 文言は `ui_text` の表に 3 つの ID（`UI_TEXT_SETTINGS_NUMBER` / `_NUMBER_ON` / `_NUMBER_OFF`）を 3 列で足す（C-018・CNF-010・CNF-011）。
   ja「行番号」「表示する」「表示しない」・en「Line numbers」「Show」「Hide」・zh-Hans「行号」「显示」「隐藏」。简体中文は母語話者の確認を得ていない（ADR 0032 と同じ）。
4. 行数は 8 → **11**。最小寸法で見える 5 行は変えず、既存のスクロール（補正 1）で下の段へ届く。
5. 変えないもの: `settings.json` の版（3 のまま）・`number` の既定値（非表示）・段の順序・application と adapters。
