...
2026-09-17（独立レビューの所見への対応を 2026-09-22 に追記）。
ゲート内の単体と、ゲート外の測定でそれぞれ何を守ったかを分ける。
...
- 拒む形 15 種を 1 件ずつ: 未知の版（2 / 0 → `UNSUPPORTED_VERSION`）、欠けたキー（`number` / `version`）、
  未知のキー（`theme`）、**重複したキー**、型の違う値（`1` / `"1"` / **`null`** / **入れ子のオブジェクト**）、
  キーの順序違い、空ファイル、閉じていない文書、末尾の余計な文字、配列。
  いずれも `MALFORMED` か `UNSUPPORTED_VERSION` で、**既定値へ落ちず、出力も触らない**
  （`null` と入れ子の 2 件は 2026-09-22 に追加）。
...
- **LF は普通の文字**（2026-09-22 追加）。`a\r\nb` は 2 行で、LF は 2 行目の 1 文字目である
  （`line_index_start(2)` は CR の次 = LF の位置）。CRLF を 2 つの区切りとして数えない。
...
- `folio_command_listed_count()` は **12**（登録表の総数 13 との差はちょうど 1 件）。
  パレットの箱の高さはこの数で決まる（2026-09-22 追加・ADR 0026 の補正 9）。
...
- 未完了の改名（`data/.rename.json` を公開済み）が残っていても、`folio_state_set_number` は
  **`rename_note` を 1 回も呼ばず**、設定だけを 1 回書く。意図はそのまま保たれ、次に `data/` を
  触る意図（保存・切替・並替・色・新規・別名保存）が再試行する（2026-09-22 追加）。
- `verify_failure_lines` が 2 つの新しい結果にも 1 行があることを確かめる。
- 確保失敗は `state_scenario` の鎖に `set_number` を足して通す。

### 分岐網羅

`python eng/coverage.py` は **92.97%（2445 / 2630 分岐）**、閾値 90%（QLT-009）。
`src/core/folio_settings.c` は 100%。閾値・除外・`eng/*.json` は触っていない。
`adopt_settings` の「`PERSISTENCE_LOADED` なのに出力を触らないポート」の分岐（ADR 0026 の補正 10）は
**単体では通らない**。偽アダプタは契約どおり常に出力を触るからで、ここは契約違反への防御である。
...
初回（57 項目）の結果は `out/design/2026-09-17/number-window-probe.log`。
独立レビューの所見に 5 項目を足して測り直した結果は `out/design/2026-09-22/number-window-probe.log` で、
**62 項目すべて成功、失敗 0、exit 0**。
...
- **`EM_STREAMIN` の最中に届く `EN_VSCROLL` は 0 回**（2026-09-22 追加・ADR 0026 の補正 5）。
  末尾まで転がしてから 200 行を流し込み、届いた `EN_VSCROLL` のたびに製品の同期描画と同じく
  `note_pane_visible_rows` を呼びにいく宿主で測った。届いた回数は 0 なので、
  **この条件では危険そのものは再現していない**。表が作られなかったことも 0 回ゆえの空振りである。
  それでも「流し込み中」の印は残す（構造で守る・補正 5 の決定どおり）。流し込みが終わったあとは
  番号が最終の本文と一致することを別に確かめた。
...
- 1 万行（10001 論理行）で桁数は 5。**本文が変わった直後の最初の組み立て 0.19 ms、
  スクロール中の 1 回の描き直し 0.19〜0.31 ms**（20 回の平均・最適化なしのビルド。
  2026-09-22 の測り直しでは 0.188 ms / 0.203 ms）。
...
- **閲覧中に `:set number` / `:set nonumber` を打っても、本文の左端も読んでいる位置も動かない**
  （2026-09-22 追加・ADR 0026 の補正 6）。40 表示行ぶん転がした状態で切り替え、
  `EM_GETFIRSTVISIBLELINE` が前後とも 40 のままであることで確かめた
  （`rearrange_keeping_line` は左端が実際に動いたときだけ論理行を戻す）。
...
8. **ヘルプが 1 行増えた最小寸法 560×360** で、一覧とキー説明が破綻しないか。
   あわせて**操作パレットの箱に空の 1 行が余っていないか**（補正 9 で高さを 12 行に直した）。
9. **番号が上の操作行（パンくず・札・閉じる）へはみ出さないか**（補正 7 の帯のクリップ）。
10. **閲覧では出ない・無題の編集では出る**こと（値では確かめていない。probe は本文の矩形でしか見ていない）。
11. `data/settings.json` を手で壊したときの、起動時の 1 行と「そのセッションは設定を変えない」振る舞い。

## 限界（この単位では測っていない・やっていない）

- **描いた絵そのものを 1 度も検査していない**。番号の字形・色・位置は実機の目視に依る。
- 帯のクリップ（`SaveDC` / `IntersectClipRect`）と、帯に掛からない更新矩形で番号の一巡を飛ばすこと
  （ADR 0026 の補正 7）は**値として測っていない**。どちらも描いた絵にしか現れず、
  `EM_POSFROMCHAR` が負の y を返す条件も相変わらず未計測である。
- 数字の幅を `refresh_font` で 1 回測る形（補正 7）は、帯の幅（4 桁 276 / 5 桁 278）が
  測り直しでも同じ値であることでしか確かめていない。**実機の DPI 変更そのものは測っていない**。
