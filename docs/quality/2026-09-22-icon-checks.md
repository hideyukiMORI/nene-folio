...
2026-09-22。ADR 0033 の検証節に対して**実際に走らせた**もの（と、走らせていないもの）をこの 1 枚に集める。
...
5. **絞り込みの欄の `×`（`paint_filter_clear`・#86）は GDI の線のまま。** ADR 0033 の決定 6 は
   ...
   この単位に足すかを決めてほしい。
   → **#93 で別 Issue にして直した**（[確認記録](2026-09-22-filter-close-checks.md)・ADR 0033 の補正 9）。
   経路は `icon_paint.c` の 1 本になり、20px の箱でも灰色に潰れないことを測った。
