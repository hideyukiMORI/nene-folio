...
| # | 何を見るか | 経路 | 結果 |
| --- | --- | --- | --- |
| 1-1 | 表の 123 値が変更前の文言と 1 字も違わない（`UI_TEXT_STATUS_PALETTE_PAGES` を除く 123 値。この 1 値は main の #69 を取り込むときに足した・ADR 0030 の補正 9） | `tests/unit/ui_text_tests.c` の期待表（`emit_test.py` が変更前のソースから生成）を ctest で回す | 成功（CTest 2/2） |
...
- 結果: **60 passed, 0 failed**（exit 0。main `d238c01` を取り込んだあとの最終 HEAD で実行）

| 節 | 測ったこと | 結果 |
| --- | --- | --- |
| 1 | 組み立てに使う表の 18 行が変更前の字面（末尾の 2 空白・`◀` `▶` `▾` を含む） | 18/18 |
| 2 | 組み立て 5 か所が変更前の連結と同じ完成形（`3 / 12 件`・`仕事-01 / 7 件`・`無題（未保存） / 0 件`・` / 0 件`（対象名なし）・` 位置 5`・`正規表現の書き方が違います。 位置 12`・`古い名前 → 新しい名前`・改名の理由 ＋ 空白 ＋ 説明・`このノート内を検索 ／ 次へ  1 / 1 件`・ヘルプのページ送り `1/2`）と、`{k}` という名前のノートが壊れないこと | 12/12 |
...
| 5 | 全 124 値が `ui_text_unit_limit`（256）に収まって UTF-16 へ写せる | 最長 **64 単位**（上限の 1/4）。2/2 |
...
## 5. main の #69 を取り込んだときの 1 件

取り込み（`chore(sync)`）で `folio_window.c` が 2 か所衝突した。どちらも**両方の変更を残す**向きで解いた
（ヘルプの行をページの先頭から数える・「操作」のページ送りを描く）。
ただし #69 の `draw_command_pages` は「1/2」を `append_number` + `"/"` + `append_number` の**連結**で作っており、
この単位で active にした C-018（語や数を連結して句を作らない）に反する。連結のまま残さず、
`UI_TEXT_STATUS_PALETTE_PAGES = "{k}/{n}"` を表に足して `ui_text_format` に通した
（表は 123 → **124 値**。ui から数を書く `append_number` は消えた）。**描かれる字面は「1/2」のまま**で、
probe の 2-12 が固定する。
