...
| CNF-012 | 同梱の書体の 1 バイトを書き換える・manifest に無いファイルを置く | eng/prove-gates.py（実 `eng/conformance.py`） | 2026-09-25: 全ファイルを写した木で `fonts/Arimo-Regular.ttf` の末尾 1 バイトを反転すると `CNF-012: fonts/Arimo-Regular.ttf: SHA-256 differs from the manifest` として非 0（P24）。`fonts/Planted-Regular.ttf` を置くと `CNF-012: fonts/Planted-Regular.ttf: not listed in the manifest` として非 0（P25）。どちらも戻すと `Conformance: 0 violation(s)` で 0 |
