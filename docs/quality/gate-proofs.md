...
7. 5 版まで積んでから拒否を `(W)` だけ（削除は許す）にして保存し、履歴の本数を数える。
...
- **ローテーションが原子的でないこと**（ADR 0012 の決定 3）も実測した。拒否を `(W)` だけ（削除は許す）にして保存すると、
  **履歴は 5 本から 4 本へ減る**（`1.md`〜`4.md` の中身と並びはそのまま・md は無傷・保存も止まる）。
  改名に `MOVEFILE_REPLACE_EXISTING` を付けたあとも**同じ結果**である。理由は Win32 の権限の分かれ方にあり、直接測って確かめた:

  | 操作 | `(W)` を拒否したディレクトリで | 意味 |
  | --- | --- | --- |
  | `DeleteFileW(5.md)` | **成功**（lastError 0） | `(W)`＝`FILE_GENERIC_WRITE` に `DELETE` は入らない。最古の版は消える |
  | `MoveFileExW(4.md → 5.md, REPLACE_EXISTING)` | **失敗**（lastError **5** = ACCESS_DENIED） | 改名は `FILE_ADD_FILE` を要るので、置き換え付きでも通らない |

  つまり「書き込みを拒否された」場合は置き換えの旗では救えない。**旗が効くのは「削除が効かなかったのに改名は許される」場合**で、
  その差は普通のディレクトリで直接測れる（`5.md` を置いたまま `4.md → 5.md` を試す）:

  | 旗 | 結果 |
  | --- | --- |
  | `MOVEFILE_WRITE_THROUGH` だけ（旧） | **失敗**（lastError **183** = ALREADY_EXISTS）。以降の `3 → 4` も相手が空かないので**連鎖が全部止まり**、`1.md` が新しい版で上書きされて直前の版を失う |
  | `MOVEFILE_REPLACE_EXISTING` を足す（現在） | **成功**。`5.md` の中身が `4.md` のものへ替わり、連鎖が最後まで走る |
- 見ていないもの: 容量不足での失敗・`data/` ごと別の機械へ写したときの履歴・外部で md を書き換えてからの保存・
