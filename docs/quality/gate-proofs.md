...
| QLT-012 | フルゲートを回していない head（Draft の push。`full-gate` が skipped）で merge を試みる | GitHub Actions の判定 job `check`（`.github/workflows/check.yml`）＋ ruleset `main` の必須 check。手元で再現できないので PR で実測 | 2026-09-23 実測（PR #146・run 35843768164）: `full-gate` = skipped・`check` = failure・`mergeStateStatus` = BLOCKED。Ready にしてフルゲートが success になると CLEAN（[確認記録](2026-09-23-required-check-checks.md)） |
...
| CNF-010 | 表示文言を `src/core/ui_text.c` の外に直書きする（非 ASCII・ワイドの `\x` エスケープ） | eng/prove-gates.py（実 `eng/conformance.py`）/ tests/conformance の text_catalog_checks 正例・反例 | 2026-09-22: 全ファイルを写した木で `CNF-010: src/ui/win32/folio_window.c: line N: display text belongs in src/core/ui_text.c`（N は末尾に足した行）として非 0（P22）。戻すと `Conformance: 0 violation(s)` で 0。非 ASCII・ワイドの `\x` エスケープは CNF-010、ナローの BOM のバイト列・文字リテラル・注記・`\\UNC\\` の `U`・`tests/` は指摘 0（tests/conformance の 10 件） |
| CNF-011 | 表示文言の表から 1 つの言語の列を消す・列を空にする | eng/prove-gates.py（実 `eng/conformance.py`）/ tests/conformance の text_column_checks 正例・反例 | 2026-09-22: 全ファイルを写した木で最後のエントリの列を 1 つ落とすと `CNF-011: src/core/ui_text.c: UI_TEXT_APP_NO_WINDOW has 2 columns, expected 3` として非 0（P23）。戻すと `Conformance: 0 violation(s)` で 0。列の欠落・空の列・`{}` でないエントリ・言語を 1 つ足した列挙は tests/conformance で CNF-011、隣接リテラルに割れた正例・注記だけのエントリ・`UI_TEXT_EMPTY` の 3 列とも空は指摘 0（tests/conformance の 10 件） |
| CNF-012 | 同梱の書体の 1 バイトを書き換える・manifest に無いファイルを置く | eng/prove-gates.py（実 `eng/conformance.py`） | 2026-09-23: 全ファイルを写した木で `fonts/NotoSansJP-Regular.otf` の末尾 1 バイトを反転すると `CNF-012: fonts/NotoSansJP-Regular.otf: SHA-256 differs from the manifest` として非 0（P24）。`fonts/Planted-Regular.ttf` を置くと `CNF-012: fonts/Planted-Regular.ttf: not listed in the manifest` として非 0（P25）。どちらも戻すと `Conformance: 0 violation(s)` で 0 |
| C-018 | 同上（CNF-010 と CNF-011 が機械強制） | 同上 | 同上（CNF-010 / CNF-011 の行と同じ実測） |
...
| P22 | CNF-010 | 表示文言を `src/core/ui_text.c` の外に直書きする | `eng/prove-gates.py`（git が知る全ファイルを写した木で実 `eng/conformance.py`） | 2026-09-22: `CNF-010: src/ui/win32/folio_window.c: line N: display text belongs in src/core/ui_text.c` で非 0。戻すと 0。許可するファイルは `textCatalog` から読むので、検出語を証明側にも直書きしない |
...
**2026-09-22 の追加（#74 / ADR 0030）**: P22（CNF-010）を足して実ツール反例は 16 本になった。`eng/prove-gates.py` は `src/ui/win32/folio_window.c` の末尾に非 ASCII のリテラルを 1 つ足して非 0 を確かめ、戻して 0 を確かめる。正例側は同じ suite の 79 テストにある。
...
### 5-m. 折り畳んだ／空のカテゴリ行に止まるカーソル（Issue #32・2026-09-11）

環境: Windows 11 Pro 10.0.26200・`build/NeNeFolio.exe`（Debug 構成＝ASan / UBSan / nullability 付き）・
主窓を `SetWindowPos` で (200, 120) へ寄せ、`GetDpiForWindow` = **120**（client 1200×800・ドロワー 300×800）。
OS のアプリのモードはダーク（`AppsUseLightTheme` は 0 のまま触っていない）。窓は `build\NeNeFolio.exe` の
プロセスの `MainWindowHandle` で選ぶ（施主が `out/hide` を同時に起動している）。
`build/data` は確認の前に `build/data.backup` へ退避し、**4 カテゴリ・ノート 27 本・全展開**
（仕事 #5B8DEF 10 本・改行 LF / 個人 #8B7CF6 9 本 CRLF / **空 #E4A11B 0 本** / 読書 #2FB8A6 8 本 CRLF）に
差し替えて、終了後に元へ戻した。**空のカテゴリを 1 つ含む**のが今回の主眼。

DPI 120 の寸法は 5-l と同じ（帯 55・カテゴリ行 43 と上の間 8・ノート行 38・下余白 20・フェード 30）。
全展開の内部の座標は 仕事 63..106 / 仕事-01〜10 が 106..486、個人 494..537 / 537..879、空 887..930（ノート行なし）、
読書 938..981 / 981..1285。下端 1285・上限 505。

手順は 5-l と同じ（索引の鍵は主窓へ `WM_KEYDOWN` を `PostMessageW` で 1 つだけ・`G` は `WM_CHAR` 0x47・
撮影は `PrintWindow(hwnd, dc, 2)` の直前に `RedrawWindow`）。**カーソルの位置はドロワーの右 60 px を走査し、
地の色（#16191D）とカテゴリ色の線形の混ぜに合う画素の塊**（8×8）で読んだ。フェードの下でも混ぜの比で見分けられる。
角は索引に区画があるとき塗り（64 画素）、無いとき枠 1px（28 画素）なので、画素数で区画も分かる。

結果:

- **`h` → `j` → `k`**: 仕事-01 を選んだ状態（角 121..128・x 272）で `h` を送ると `categories.json` の 仕事 が
  `expanded: false` になり、**角は 仕事 のカテゴリ行へ移った**（80..87・x 256＝`+` の左）。右ペインは 仕事-01 のまま。
  `j` で 個人-01 が選ばれて右ペインが変わり（角 172）、`k` で**また 仕事 のカテゴリ行に止まり、右ペインは 個人-01 のまま**
  （[cursor-collapsed.png](cursor-collapsed.png)）
- **`l` の移り先**: その状態で `l` を送ると 仕事 が `true` に戻り、選択（個人-01）はその中に無いので
  **最初のノート 仕事-01 が選ばれて右ペインが変わった**（角 121・[cursor-expanded.png](cursor-expanded.png)）。
  続けて `j` `j` で 仕事-03（角 197）を選び、`h` で畳むと角はカテゴリ行（80）へ、`l` で**元の 仕事-03（197）へ戻る**
  （選択が中にあるときは最初のノートへは行かない）。右ペインは 仕事-03 のまま動かない
- **空のカテゴリ**: 個人-09 から `j` を送ると**ノートを 1 本も持たない 空 の行に止まった**（角 419・色 E4A11B・
  [cursor-empty.png](cursor-empty.png)）。そこで `l` を送っても `categories.json` の mtime は動かず（既に展開）、
  `h` で `expanded: false`・`l` で `true` に戻るあいだ**角は 空 の行から動かない**。**`Enter` を送っても
  フォーカスは主窓のまま**（`GetGUIThreadInfo` の `hwndFocus` が主窓 = 索引。RichEdit へは移らない）
- **`G` の末尾が折り畳み**: 読書-01 で `h` を送って 読書 を畳み（角は 読書 の行）、`gg` で 仕事-01 へ戻ってから `G` を送ると、
  **末尾の 読書 の行にカーソルが止まり、ドロワーが 181 px スクロールして**その行が下端に来た
  （角 774..781＝(938+981)/2 − 4 − 181・[cursor-tail.png](cursor-tail.png)）。右ペインは 仕事-01 のまま
- **`h` / `l` のあとも寄せる**（2026-09-11 の追補）: 下端に来ている折り畳んだ 読書 の行（角 774・量 181）で `l` を送ると、
  中の 読書-01 が選ばれて**量が 219 へ動き、その行が下端に収まった**（角 777＝(981+1019)/2 − 4 − 219）。
  寄せる前は 読書-01 の行（内部 981..1019）が画面の外に出ていた
- **編集のまま**: 仕事-01 を「編集」の札で編集にして末尾に `ABC` を打ち（RichEdit の長さ 23 → 26・md は 36 バイトのまま）、
  `h` を送ると**角はカテゴリ行へ移るが `仕事-01.md` は 36 バイト・mtime も不変**。続けて `j` で 個人-01（ノート行）へ動くと
  **そこで初めて保存**され 39 バイトになり、`.history/仕事/仕事-01/1.md` に保存前の 36 バイトが積まれた。
  個人-01 を編集のまま `PQR` を打って（26）**`k` で折り畳んだ 仕事 の行に止まっても `個人-01.md` は 39 バイトのまま**
  （[cursor-edit.png](cursor-edit.png)・`ES_READONLY` は落ちたまま＝編集モードのまま）。そこから `j` で 個人-01 へ戻ると
  42 バイトになり履歴も 1 つ積まれた
- **カテゴリ行のクリック**: 個人-01 にカーソルがある状態で 空 のカテゴリ行を 2 回クリックすると、`categories.json` の
  `expanded` が `false` → `true` と書き戻るが、**角は 個人-01 の行から動かず**右ペインも変わらない（ADR 0015 の決定 5）
- 見ていないもの: 96 DPI と高 DPI（144 以上）での角の位置・**実際のキーボードでの打鍵**（合成した鍵だけを見た）・
  ライトのアプリのモードでの角の見え方・IME が ON のときの `j`（ADR 0013 の「失う・残る」のまま）・
  カーソルがカテゴリ行にあるときのドラッグとの取り合わせ

## 日本語GUI入口の確認範囲（#49、2026-09-13）

手順と未確認条件は [GUI入口の実機記録](2026-09-13-gui-entry-checks.md)。Computer Useのnative pipe接続が失敗しており、現時点でこの変更の実機合格は主張しない。以前のexeの確認やChromeの画面案を流用しない。
