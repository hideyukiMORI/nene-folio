...
- **ローテーションが原子的でないこと**（ADR 0012 の決定 3）も 1 度実測した: 拒否を `(W)` だけ（削除は許す）にすると、
  `5.md` の `DeleteFileW` は通り、続く `4.md → 5.md` の `MoveFileExW` が落ちて **履歴が 5 本から 4 本へ減った**。
  md は無傷で、保存も止まる。「番号が欠けた履歴が残りうる」という決定 3 のとおりである
