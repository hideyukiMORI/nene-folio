...
| QLT-012 | フルゲートを回していない head（Draft の push。`full-gate` が skipped）で merge を試みる | GitHub Actions の判定 job `check`（`.github/workflows/check.yml`）＋ ruleset `main` の必須 check。手元で再現できないので PR で実測 | 2026-09-26 実測（PR #146・run 35843768164）: `full-gate` = skipped・`check` = failure・`mergeStateStatus` = BLOCKED。Ready にしてフルゲートが success になると CLEAN（[確認記録](2026-09-26-required-check-checks.md)） |
...
| CNF-011 | 表示文言の表から 1 つの言語の列を消す・列を空にする | eng/prove-gates.py（実 `eng/conformance.py`）/ tests/conformance の text_column_checks 正例・反例 | 2026-09-23: 全ファイルを写した木で最後のエントリの列を 1 つ落とすと `CNF-011: src/core/ui_text.c: UI_TEXT_APP_NO_WINDOW has 2 columns, expected 3` として非 0（P23）。戻すと `Conformance: 0 violation(s)` で 0。列の欠落・空の列・`{}` でないエントリ・言語を 1 つ足した列挙は tests/conformance で CNF-011、隣接リテラルに割れた正例・注記だけのエントリ・`UI_TEXT_EMPTY` の 3 列とも空は指摘 0（tests/conformance の 10 件） |
| CNF-012 | 同梱の書体の 1 バイトを書き換える・manifest に無いファイルを置く | eng/prove-gates.py（実 `eng/conformance.py`） | 2026-09-25: 全ファイルを写した木で `fonts/NotoSansJP-Regular.otf` の末尾 1 バイトを反転すると `CNF-012: fonts/NotoSansJP-Regular.otf: SHA-256 differs from the manifest` として非 0（P24）。`fonts/Planted-Regular.ttf` を置くと `CNF-012: fonts/Planted-Regular.ttf: not listed in the manifest` として非 0（P25）。どちらも戻すと `Conformance: 0 violation(s)` で 0 |
