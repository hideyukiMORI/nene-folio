...
比較のために**修正前の実行ファイル**（HEAD `85268bc` = production コードは main の `a860218` と同じ）を
`build/NeNeFolio-before.exe` として並べて置いた（確認後に消した）。撮影する側も Per-Monitor v2（5-e の教訓）。
...
- **窓の外形と client**: 修正後は `GetWindowRect` と `GetClientRect` + `ClientToScreen` の差が
  ...
  （[frame-after.png](frame-after.png)）。修正前も同じ条件では帯が無かった（[frame-before.png](frame-before.png)）
- **白い帯は再現しなかった（ADR 0011 の「文脈」(1) の原因の特定は、この機械では確認できていない）**:
  修正前・修正後の**どちらも**、窓が現れてから約 **37 ms** のあいだ client は **1184×784**（窓は 1200×800・差 8 px）で、
  そのあと 1200×800 になる。修正後も同じということは、`WM_NCCALCSIZE` は `CreateWindowExW` の中では送られておらず、
  最初に届くのは `GWLP_USERDATA` を結んだあと（`ShowWindow` のとき）だという意味である。つまり
  「最初の `WM_NCCALCSIZE` が `DefWindowProcW` に流れる」は、この機械では起きていない。決定 1 が実測で効いているのは
  **最大化**（次の項）で、結び付けの前に答えるのは防御として残る。
  ただし 1 度だけ、修正前の実行ファイルで client が内側のまま 1 回描かせたあと、1.5 秒後の写真の縁 8 px が
  **左端・上端とも `#E3E3E3` `#FFFFFF` `#F4F7FC`×6**（＝白い帯）になり、`GetClientRect` が 1200×800 を返しているのに
  帯が残るのを観測した。同じ手順を修正前 12 回・修正後 6 回繰り返しても再現しなかったので、**発生条件は掴めていない**
...
- **最小サイズ**: 300×200 を頼むと、修正前は **300×200 になる**。修正後は **DPI 120 で 700×450・DPI 144 で 840×540** で
  止まる（96 DPI の 560×360 の 1.25 倍 / 1.5 倍・`MulDiv` の丸めどおり）
...
- 見ていないもの: **96 DPI**（この機械の 4 枚のモニタは 120 / 144 / 168 / 168 で 100% のモニタが無い。施主に依頼）。
  ...
  白い帯の再現条件と、施主の実機で直ったかどうか。リサイズ直後の残像（`PrintWindow` では写らない）。
  ライトのアプリのモードでの見え方（`AppsUseLightTheme` は 0 のまま触っていない）。
