...
- 中核の分岐 998/1052（94.87%）。確保失敗の経路は測定ビルドの注入で全部走る

## 動かないもの

- 編集モードと md の保存（FR-006）・リンクを開く・選択行の強調表示
- ドラッグの並び替え・右クリックの色変更・検索・ドロワーのスクロール
- `index.json` の保存（起動時の照合結果は書き戻さない）
- DPI 切り替え・スナップ・縁のリサイズは実装したが実機で未確認

## 次の 1 手

デザイン案（`/design`）を作り、ドロワーと右ペインの見た目（配色・字体・余白・選択行・掴む帯）をそれに合わせる。そのあと編集モードと保存（FR-006）。
