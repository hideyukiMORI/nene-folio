...
try {
    ...
    $files = @(& git ls-files --cached --others --exclude-standard -- '*.c' '*.h') | Sort-Object -Unique
    ...
    Write-Host 'NeNe Folio full gate passed (UI hardware checks are recorded separately).'
}
