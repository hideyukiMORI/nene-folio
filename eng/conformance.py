"""Repository conformance checks for NeNe Folio; Python standard library only (Issue #1)."""
...
RULE_ID = r"(?:ARC|C|QLT|CNF|GIT)-\d{3}"
...
def c_code(text: str) -> str:
    # Preserve newlines while masking comments and quoted strings/characters.
    pattern = r'//[^\n]*|/\*.*?\*/|"(?:\\.|[^"\\])*"|\'(?:\\.|[^\'\\])*\''
    return re.sub(pattern, lambda m: re.sub(r"[^\n]", " ", m.group()), text, flags=re.S)


def top_level_types(code: str) -> list[str]:
    """Names of struct/enum/union definitions and typedef names at brace depth 0."""
    names = []
    depth = 0
    typedef_pending = False
    token = r"\btypedef\b|\b(?:struct|enum|union)\s*([A-Za-z_]\w*)?\s*(?::[^;{}]+)?\{|\b([A-Za-z_]\w*)\s*(?:\[[^\]]*\]\s*)?;|[{}]"
    for match in re.finditer(token, code):
        text = match[0]
        if text == "typedef":
            typedef_pending = depth == 0
        elif text == "{":
            depth += 1
        elif text == "}":
            depth = max(0, depth - 1)
            if depth == 0 and typedef_pending:
                tail = re.match(r"\s*([A-Za-z_]\w*)\s*(?:\[[^\]]*\]\s*)?;", code[match.end():])
                if tail:
                    names.append(tail[1])
                    typedef_pending = False
        elif match[1] is not None or text.rstrip().endswith("{"):
            if depth == 0 and match[1]:
                names.append(match[1])
            depth += 1
        elif match[2] and depth == 0 and typedef_pending:
            names.append(match[2])
            typedef_pending = False
        elif text.endswith(";") and depth == 0:
            typedef_pending = False
    return names


def external_functions(code: str) -> list[str]:
    """Names of non-static function definitions at brace depth 0."""
    names = []
    depth = 0
    token = r"^[ \t]*((?:[A-Za-z_][\w\s\*]*?)\b([A-Za-z_]\w*)\s*\([^;{}]*\)\s*\{)|[{}]"
    for match in re.finditer(token, code, re.M):
        text = match[0]
        if text == "{":
            depth += 1
        elif text == "}":
            ...
            head = match[1]
            if depth == 0 and not re.match(r"\s*static\b", head) and not re.search(r"\b(?:if|for|while|switch|return)\b", head):
                names.append(match[2])
            depth += 1
    return names
...
def source_checks(path: str, text: str, rules: dict, waivers: dict) -> list[Finding]:
    ...
    if production:
        ...
        for name in top_level_types(code):
            if generic_name(name, rules["typeSuffixes"]):
                findings.append(Finding("CNF-001", path, f"forbidden type name {name}"))
        definitions = top_level_types(code)
        if len(set(definitions)) > 1:
            findings.append(Finding("CNF-002", path, "multiple top-level type definitions"))
        if definitions and definitions[0] != stem:
            findings.append(Finding("CNF-002", path, f"filename does not match {definitions[0]}"))
        if not path.startswith("src/app/"):
            for function in external_functions(code):
                if not function.startswith(stem + "_"):
                    findings.append(Finding("CNF-002", path, f"external function {function} lacks the {stem}_ prefix"))
    ...
    code_lines = c_code(text).splitlines()
...
def configuration_checks(root: Path, paths: list[Path], rules: dict) -> list[Finding]:
    ...
    for path in paths:
        ...
        if path.suffix in {".c", ".h", ".ps1", ".py", ".cmake"} and not name.startswith("tests/conformance/"):
...
def architecture_checks(root: Path, paths: list[Path], build_dir: Path | None) -> list[Finding]:
    ...
    source_paths = {p.as_posix() for p in paths if p.suffix in {".c", ".h"}}
    ...
    for config in model["configurations"]:
        ...
        for entry in config["targets"]:
            ...
            compiled.update(s for s in sources if Path(s).suffix == ".c")
    ...
    for source in source_paths:
        if source.startswith(("src/", "tests/build/", "tests/unit/")) and Path(source).suffix == ".c" and source not in compiled:
