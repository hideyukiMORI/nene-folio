...
/* anchor から次の一致を探す。**前方なら anchor の開始の次**（anchor が空＝caret ならその位置）、
 * 後方なら anchor.start より前から探し、端に達したら反対の端から続けて（巡回）
 * 候補の開始位置をちょうど 1 周ぶん見る。重なった一致も飛ばさないので、前方に辿っても後方に辿っても
 * note_search_count が数える一致を全部通る（2026-09-17 の補正）。
 * 見つかれば out に一致の範囲（end = start + 語の長さ）を書いて FOUND。
 * anchor.start が anchor.end より後ろなら BAD_SPAN、語が空なら NO_TERM、
 * 本文か語が整形式の UTF-16 でなければ MALFORMED、一致が無ければ NOT_FOUND で、いずれも out
 * は触らない。 本文と語が整形式なら、一致がサロゲート対を跨いで始まったり終わったりすることはない。
 */
 ...
 * **position 以下から始まる一致の数**を書く。position が一致の開始位置ならその 1 始まりの順番で、
 * 一致の開始でない位置でもそこまでに現れた一致の数を返す（契約であり、丸めや推測ではない）。
